...
const ChatContextProvider = (props) => {
    // Declaring hooks to maintain state
    ...
    // method to add next admin message
    ...
    const addNextAdminMessage = () => {
        ...
        setTimeout(() => {
            ...
            setMessages(messages => {
                const userMessageCount = messages.reduce((count, message) => {   //redue to subtract the number (count - message)
                    if (!message.isAdmin) {
                        count++;
                    }
                    return count;
    ...
    //when user clicks yes this method adds the user message to existing message
